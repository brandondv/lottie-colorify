(function (_, I) {
  typeof exports == 'object' && typeof module < 'u'
    ? I(exports)
    : typeof define == 'function' && define.amd
      ? define(['exports'], I)
      : ((_ = typeof globalThis < 'u' ? globalThis : _ || self), I((_.index = {})));
})(this, function (_) {
  'use strict';
  const I = (t) => {
      if (typeof t == 'string' && t.match(/^#?([a-f\d]{2})([a-f\d]{2})([a-f\d]{2})$/i)) {
        const e = /^#?([a-f\d]{2})([a-f\d]{2})([a-f\d]{2})$/i.exec(t);
        if (!e) throw new Error('Color can be only hex or rgb array (ex. [10,20,30])');
        return [
          Math.round((parseInt(e[1], 16) / 255) * 1e3) / 1e3,
          Math.round((parseInt(e[2], 16) / 255) * 1e3) / 1e3,
          Math.round((parseInt(e[3], 16) / 255) * 1e3) / 1e3,
          1,
        ];
      } else {
        if (typeof t == 'object' && t.length === 3 && t.every((e) => e >= 0 && e <= 255))
          return [
            Math.round((t[0] / 255) * 1e3) / 1e3,
            Math.round((t[1] / 255) * 1e3) / 1e3,
            Math.round((t[2] / 255) * 1e3) / 1e3,
            1,
          ];
        if (typeof t == 'object' && t.length === 4 && t.every((e) => e >= 0 && e <= 255))
          return [
            Math.round((t[0] / 255) * 1e3) / 1e3,
            Math.round((t[1] / 255) * 1e3) / 1e3,
            Math.round((t[2] / 255) * 1e3) / 1e3,
            t[3],
          ];
        throw t
          ? new Error('Color can be only hex or rgb(a) array (ex. [10,20,30,1])')
          : new Error('Color not defined');
      }
    },
    ke = (t) => {
      const e = Math.round(t[0] * 255)
          .toString(16)
          .padStart(2, '0'),
        a = Math.round(t[1] * 255)
          .toString(16)
          .padStart(2, '0'),
        o = Math.round(t[2] * 255)
          .toString(16)
          .padStart(2, '0');
      return `#${e}${a}${o}`;
    },
    Qt = (t) => [Math.round(t[0] * 255), Math.round(t[1] * 255), Math.round(t[2] * 255)],
    Tt = (t) => [Math.round(t[0] * 255), Math.round(t[1] * 255), Math.round(t[2] * 255), t[3]],
    tr = (t) => {
      const e = [];
      function a(o) {
        if (o.g && o.g.k && o.g.k.k && Array.isArray(o.g.k.k) && o.g.k.k.length % 4 === 0) {
          Array.isArray(o.g.k.k) && typeof o.g.k.k[0] != 'number' && a(o.g.k.k);
          const c = [...o.g.k.k];
          for (; c.length; ) {
            const l = [...c.splice(1, 3), c.shift()];
            e.push(Tt(l));
          }
        } else if (o.s && Array.isArray(o.s) && o.s.length === 4) e.push(Tt(o.s));
        else if (o.c && o.c.k) Array.isArray(o.c.k) && typeof o.c.k[0] != 'number' ? a(o.c.k) : e.push(Qt(o.c.k));
        else for (const c in o) typeof o[c] == 'object' && a(o[c]);
        return e;
      }
      return a(t), e;
    },
    Oe = (t) => {
      const a = tr(t).map((c) => JSON.stringify(c)),
        o = new Set(a);
      return Array.from(o, (c) => JSON.parse(c));
    };
  var E =
    typeof globalThis < 'u'
      ? globalThis
      : typeof window < 'u'
        ? window
        : typeof global < 'u'
          ? global
          : typeof self < 'u'
            ? self
            : {};
  function rr(t) {
    return t && t.__esModule && Object.prototype.hasOwnProperty.call(t, 'default') ? t.default : t;
  }
  var at = { exports: {} };
  at.exports,
    (function (t, e) {
      var a = 200,
        o = '__lodash_hash_undefined__',
        c = 9007199254740991,
        l = '[object Arguments]',
        i = '[object Array]',
        p = '[object Boolean]',
        g = '[object Date]',
        f = '[object Error]',
        d = '[object Function]',
        A = '[object GeneratorFunction]',
        C = '[object Map]',
        Z = '[object Number]',
        Q = '[object Object]',
        tt = '[object Promise]',
        w = '[object RegExp]',
        S = '[object Set]',
        Yr = '[object String]',
        Xr = '[object Symbol]',
        Nt = '[object WeakMap]',
        Zr = '[object ArrayBuffer]',
        ht = '[object DataView]',
        Qr = '[object Float32Array]',
        te = '[object Float64Array]',
        re = '[object Int8Array]',
        ee = '[object Int16Array]',
        ae = '[object Int32Array]',
        ne = '[object Uint8Array]',
        oe = '[object Uint8ClampedArray]',
        se = '[object Uint16Array]',
        ie = '[object Uint32Array]',
        bl = /[\\^$.*+?()[\]{}|]/g,
        Tl = /\w*$/,
        Al = /^\[object .+?Constructor\]$/,
        Sl = /^(?:0|[1-9]\d*)$/,
        y = {};
      (y[l] =
        y[i] =
        y[Zr] =
        y[ht] =
        y[p] =
        y[g] =
        y[Qr] =
        y[te] =
        y[re] =
        y[ee] =
        y[ae] =
        y[C] =
        y[Z] =
        y[Q] =
        y[w] =
        y[S] =
        y[Yr] =
        y[Xr] =
        y[ne] =
        y[oe] =
        y[se] =
        y[ie] =
          !0),
        (y[f] = y[d] = y[Nt] = !1);
      var wl = typeof E == 'object' && E && E.Object === Object && E,
        ml = typeof self == 'object' && self && self.Object === Object && self,
        x = wl || ml || Function('return this')(),
        ce = e && !e.nodeType && e,
        ue = ce && !0 && t && !t.nodeType && t,
        Cl = ue && ue.exports === ce;
      function Il(r, n) {
        return r.set(n[0], n[1]), r;
      }
      function Ml(r, n) {
        return r.add(n), r;
      }
      function xl(r, n) {
        for (var s = -1, u = r ? r.length : 0; ++s < u && n(r[s], s, r) !== !1; );
        return r;
      }
      function Pl(r, n) {
        for (var s = -1, u = n.length, $ = r.length; ++s < u; ) r[$ + s] = n[s];
        return r;
      }
      function fe(r, n, s, u) {
        for (var $ = -1, b = r ? r.length : 0; ++$ < b; ) s = n(s, r[$], $, r);
        return s;
      }
      function jl(r, n) {
        for (var s = -1, u = Array(r); ++s < r; ) u[s] = n(s);
        return u;
      }
      function El(r, n) {
        return r == null ? void 0 : r[n];
      }
      function le(r) {
        var n = !1;
        if (r != null && typeof r.toString != 'function')
          try {
            n = !!(r + '');
          } catch {}
        return n;
      }
      function pe(r) {
        var n = -1,
          s = Array(r.size);
        return (
          r.forEach(function (u, $) {
            s[++n] = [$, u];
          }),
          s
        );
      }
      function Kt(r, n) {
        return function (s) {
          return r(n(s));
        };
      }
      function ge(r) {
        var n = -1,
          s = Array(r.size);
        return (
          r.forEach(function (u) {
            s[++n] = u;
          }),
          s
        );
      }
      var kl = Array.prototype,
        Ol = Function.prototype,
        yt = Object.prototype,
        Rt = x['__core-js_shared__'],
        he = (function () {
          var r = /[^.]+$/.exec((Rt && Rt.keys && Rt.keys.IE_PROTO) || '');
          return r ? 'Symbol(src)_1.' + r : '';
        })(),
        ye = Ol.toString,
        k = yt.hasOwnProperty,
        vt = yt.toString,
        Dl = RegExp(
          '^' +
            ye
              .call(k)
              .replace(bl, '\\$&')
              .replace(/hasOwnProperty|(function).*?(?=\\\()| for .+?(?=\\\])/g, '$1.*?') +
            '$',
        ),
        ve = Cl ? x.Buffer : void 0,
        de = x.Symbol,
        _e = x.Uint8Array,
        Gl = Kt(Object.getPrototypeOf, Object),
        Fl = Object.create,
        Bl = yt.propertyIsEnumerable,
        Ll = kl.splice,
        $e = Object.getOwnPropertySymbols,
        Ul = ve ? ve.isBuffer : void 0,
        Nl = Kt(Object.keys, Object),
        Ht = q(x, 'DataView'),
        rt = q(x, 'Map'),
        Vt = q(x, 'Promise'),
        zt = q(x, 'Set'),
        qt = q(x, 'WeakMap'),
        et = q(Object, 'create'),
        Kl = B(Ht),
        Rl = B(rt),
        Hl = B(Vt),
        Vl = B(zt),
        zl = B(qt),
        be = de ? de.prototype : void 0,
        Te = be ? be.valueOf : void 0;
      function G(r) {
        var n = -1,
          s = r ? r.length : 0;
        for (this.clear(); ++n < s; ) {
          var u = r[n];
          this.set(u[0], u[1]);
        }
      }
      function ql() {
        this.__data__ = et ? et(null) : {};
      }
      function Wl(r) {
        return this.has(r) && delete this.__data__[r];
      }
      function Jl(r) {
        var n = this.__data__;
        if (et) {
          var s = n[r];
          return s === o ? void 0 : s;
        }
        return k.call(n, r) ? n[r] : void 0;
      }
      function Yl(r) {
        var n = this.__data__;
        return et ? n[r] !== void 0 : k.call(n, r);
      }
      function Xl(r, n) {
        var s = this.__data__;
        return (s[r] = et && n === void 0 ? o : n), this;
      }
      (G.prototype.clear = ql),
        (G.prototype.delete = Wl),
        (G.prototype.get = Jl),
        (G.prototype.has = Yl),
        (G.prototype.set = Xl);
      function P(r) {
        var n = -1,
          s = r ? r.length : 0;
        for (this.clear(); ++n < s; ) {
          var u = r[n];
          this.set(u[0], u[1]);
        }
      }
      function Zl() {
        this.__data__ = [];
      }
      function Ql(r) {
        var n = this.__data__,
          s = dt(n, r);
        if (s < 0) return !1;
        var u = n.length - 1;
        return s == u ? n.pop() : Ll.call(n, s, 1), !0;
      }
      function tp(r) {
        var n = this.__data__,
          s = dt(n, r);
        return s < 0 ? void 0 : n[s][1];
      }
      function rp(r) {
        return dt(this.__data__, r) > -1;
      }
      function ep(r, n) {
        var s = this.__data__,
          u = dt(s, r);
        return u < 0 ? s.push([r, n]) : (s[u][1] = n), this;
      }
      (P.prototype.clear = Zl),
        (P.prototype.delete = Ql),
        (P.prototype.get = tp),
        (P.prototype.has = rp),
        (P.prototype.set = ep);
      function V(r) {
        var n = -1,
          s = r ? r.length : 0;
        for (this.clear(); ++n < s; ) {
          var u = r[n];
          this.set(u[0], u[1]);
        }
      }
      function ap() {
        this.__data__ = { hash: new G(), map: new (rt || P)(), string: new G() };
      }
      function np(r) {
        return _t(this, r).delete(r);
      }
      function op(r) {
        return _t(this, r).get(r);
      }
      function sp(r) {
        return _t(this, r).has(r);
      }
      function ip(r, n) {
        return _t(this, r).set(r, n), this;
      }
      (V.prototype.clear = ap),
        (V.prototype.delete = np),
        (V.prototype.get = op),
        (V.prototype.has = sp),
        (V.prototype.set = ip);
      function z(r) {
        this.__data__ = new P(r);
      }
      function cp() {
        this.__data__ = new P();
      }
      function up(r) {
        return this.__data__.delete(r);
      }
      function fp(r) {
        return this.__data__.get(r);
      }
      function lp(r) {
        return this.__data__.has(r);
      }
      function pp(r, n) {
        var s = this.__data__;
        if (s instanceof P) {
          var u = s.__data__;
          if (!rt || u.length < a - 1) return u.push([r, n]), this;
          s = this.__data__ = new V(u);
        }
        return s.set(r, n), this;
      }
      (z.prototype.clear = cp),
        (z.prototype.delete = up),
        (z.prototype.get = fp),
        (z.prototype.has = lp),
        (z.prototype.set = pp);
      function gp(r, n) {
        var s = Yt(r) || Fp(r) ? jl(r.length, String) : [],
          u = s.length,
          $ = !!u;
        for (var b in r) k.call(r, b) && !($ && (b == 'length' || kp(b, u))) && s.push(b);
        return s;
      }
      function Ae(r, n, s) {
        var u = r[n];
        (!(k.call(r, n) && Ce(u, s)) || (s === void 0 && !(n in r))) && (r[n] = s);
      }
      function dt(r, n) {
        for (var s = r.length; s--; ) if (Ce(r[s][0], n)) return s;
        return -1;
      }
      function hp(r, n) {
        return r && Se(n, Xt(n), r);
      }
      function Wt(r, n, s, u, $, b, j) {
        var T;
        if ((u && (T = b ? u(r, $, b, j) : u(r)), T !== void 0)) return T;
        if (!$t(r)) return r;
        var xe = Yt(r);
        if (xe) {
          if (((T = Pp(r)), !n)) return Ip(r, T);
        } else {
          var W = F(r),
            Pe = W == d || W == A;
          if (Lp(r)) return bp(r, n);
          if (W == Q || W == l || (Pe && !b)) {
            if (le(r)) return b ? r : {};
            if (((T = jp(Pe ? {} : r)), !n)) return Mp(r, hp(T, r));
          } else {
            if (!y[W]) return b ? r : {};
            T = Ep(r, W, Wt, n);
          }
        }
        j || (j = new z());
        var je = j.get(r);
        if (je) return je;
        if ((j.set(r, T), !xe)) var Ee = s ? xp(r) : Xt(r);
        return (
          xl(Ee || r, function (Zt, bt) {
            Ee && ((bt = Zt), (Zt = r[bt])), Ae(T, bt, Wt(Zt, n, s, u, bt, r, j));
          }),
          T
        );
      }
      function yp(r) {
        return $t(r) ? Fl(r) : {};
      }
      function vp(r, n, s) {
        var u = n(r);
        return Yt(r) ? u : Pl(u, s(r));
      }
      function dp(r) {
        return vt.call(r);
      }
      function _p(r) {
        if (!$t(r) || Dp(r)) return !1;
        var n = Me(r) || le(r) ? Dl : Al;
        return n.test(B(r));
      }
      function $p(r) {
        if (!me(r)) return Nl(r);
        var n = [];
        for (var s in Object(r)) k.call(r, s) && s != 'constructor' && n.push(s);
        return n;
      }
      function bp(r, n) {
        if (n) return r.slice();
        var s = new r.constructor(r.length);
        return r.copy(s), s;
      }
      function Jt(r) {
        var n = new r.constructor(r.byteLength);
        return new _e(n).set(new _e(r)), n;
      }
      function Tp(r, n) {
        var s = n ? Jt(r.buffer) : r.buffer;
        return new r.constructor(s, r.byteOffset, r.byteLength);
      }
      function Ap(r, n, s) {
        var u = n ? s(pe(r), !0) : pe(r);
        return fe(u, Il, new r.constructor());
      }
      function Sp(r) {
        var n = new r.constructor(r.source, Tl.exec(r));
        return (n.lastIndex = r.lastIndex), n;
      }
      function wp(r, n, s) {
        var u = n ? s(ge(r), !0) : ge(r);
        return fe(u, Ml, new r.constructor());
      }
      function mp(r) {
        return Te ? Object(Te.call(r)) : {};
      }
      function Cp(r, n) {
        var s = n ? Jt(r.buffer) : r.buffer;
        return new r.constructor(s, r.byteOffset, r.length);
      }
      function Ip(r, n) {
        var s = -1,
          u = r.length;
        for (n || (n = Array(u)); ++s < u; ) n[s] = r[s];
        return n;
      }
      function Se(r, n, s, u) {
        s || (s = {});
        for (var $ = -1, b = n.length; ++$ < b; ) {
          var j = n[$],
            T = void 0;
          Ae(s, j, T === void 0 ? r[j] : T);
        }
        return s;
      }
      function Mp(r, n) {
        return Se(r, we(r), n);
      }
      function xp(r) {
        return vp(r, Xt, we);
      }
      function _t(r, n) {
        var s = r.__data__;
        return Op(n) ? s[typeof n == 'string' ? 'string' : 'hash'] : s.map;
      }
      function q(r, n) {
        var s = El(r, n);
        return _p(s) ? s : void 0;
      }
      var we = $e ? Kt($e, Object) : Kp,
        F = dp;
      ((Ht && F(new Ht(new ArrayBuffer(1))) != ht) ||
        (rt && F(new rt()) != C) ||
        (Vt && F(Vt.resolve()) != tt) ||
        (zt && F(new zt()) != S) ||
        (qt && F(new qt()) != Nt)) &&
        (F = function (r) {
          var n = vt.call(r),
            s = n == Q ? r.constructor : void 0,
            u = s ? B(s) : void 0;
          if (u)
            switch (u) {
              case Kl:
                return ht;
              case Rl:
                return C;
              case Hl:
                return tt;
              case Vl:
                return S;
              case zl:
                return Nt;
            }
          return n;
        });
      function Pp(r) {
        var n = r.length,
          s = r.constructor(n);
        return n && typeof r[0] == 'string' && k.call(r, 'index') && ((s.index = r.index), (s.input = r.input)), s;
      }
      function jp(r) {
        return typeof r.constructor == 'function' && !me(r) ? yp(Gl(r)) : {};
      }
      function Ep(r, n, s, u) {
        var $ = r.constructor;
        switch (n) {
          case Zr:
            return Jt(r);
          case p:
          case g:
            return new $(+r);
          case ht:
            return Tp(r, u);
          case Qr:
          case te:
          case re:
          case ee:
          case ae:
          case ne:
          case oe:
          case se:
          case ie:
            return Cp(r, u);
          case C:
            return Ap(r, u, s);
          case Z:
          case Yr:
            return new $(r);
          case w:
            return Sp(r);
          case S:
            return wp(r, u, s);
          case Xr:
            return mp(r);
        }
      }
      function kp(r, n) {
        return (n = n ?? c), !!n && (typeof r == 'number' || Sl.test(r)) && r > -1 && r % 1 == 0 && r < n;
      }
      function Op(r) {
        var n = typeof r;
        return n == 'string' || n == 'number' || n == 'symbol' || n == 'boolean' ? r !== '__proto__' : r === null;
      }
      function Dp(r) {
        return !!he && he in r;
      }
      function me(r) {
        var n = r && r.constructor,
          s = (typeof n == 'function' && n.prototype) || yt;
        return r === s;
      }
      function B(r) {
        if (r != null) {
          try {
            return ye.call(r);
          } catch {}
          try {
            return r + '';
          } catch {}
        }
        return '';
      }
      function Gp(r) {
        return Wt(r, !0, !0);
      }
      function Ce(r, n) {
        return r === n || (r !== r && n !== n);
      }
      function Fp(r) {
        return Bp(r) && k.call(r, 'callee') && (!Bl.call(r, 'callee') || vt.call(r) == l);
      }
      var Yt = Array.isArray;
      function Ie(r) {
        return r != null && Up(r.length) && !Me(r);
      }
      function Bp(r) {
        return Np(r) && Ie(r);
      }
      var Lp = Ul || Rp;
      function Me(r) {
        var n = $t(r) ? vt.call(r) : '';
        return n == d || n == A;
      }
      function Up(r) {
        return typeof r == 'number' && r > -1 && r % 1 == 0 && r <= c;
      }
      function $t(r) {
        var n = typeof r;
        return !!r && (n == 'object' || n == 'function');
      }
      function Np(r) {
        return !!r && typeof r == 'object';
      }
      function Xt(r) {
        return Ie(r) ? gp(r) : $p(r);
      }
      function Kp() {
        return [];
      }
      function Rp() {
        return !1;
      }
      t.exports = Gp;
    })(at, at.exports);
  var De = at.exports;
  const At = rr(De);
  function Ge() {
    (this.__data__ = []), (this.size = 0);
  }
  var Fe = Ge;
  function Be(t, e) {
    return t === e || (t !== t && e !== e);
  }
  var er = Be,
    Le = er;
  function Ue(t, e) {
    for (var a = t.length; a--; ) if (Le(t[a][0], e)) return a;
    return -1;
  }
  var nt = Ue,
    Ne = nt,
    Ke = Array.prototype,
    Re = Ke.splice;
  function He(t) {
    var e = this.__data__,
      a = Ne(e, t);
    if (a < 0) return !1;
    var o = e.length - 1;
    return a == o ? e.pop() : Re.call(e, a, 1), --this.size, !0;
  }
  var Ve = He,
    ze = nt;
  function qe(t) {
    var e = this.__data__,
      a = ze(e, t);
    return a < 0 ? void 0 : e[a][1];
  }
  var We = qe,
    Je = nt;
  function Ye(t) {
    return Je(this.__data__, t) > -1;
  }
  var Xe = Ye,
    Ze = nt;
  function Qe(t, e) {
    var a = this.__data__,
      o = Ze(a, t);
    return o < 0 ? (++this.size, a.push([t, e])) : (a[o][1] = e), this;
  }
  var ta = Qe,
    ra = Fe,
    ea = Ve,
    aa = We,
    na = Xe,
    oa = ta;
  function L(t) {
    var e = -1,
      a = t == null ? 0 : t.length;
    for (this.clear(); ++e < a; ) {
      var o = t[e];
      this.set(o[0], o[1]);
    }
  }
  (L.prototype.clear = ra),
    (L.prototype.delete = ea),
    (L.prototype.get = aa),
    (L.prototype.has = na),
    (L.prototype.set = oa);
  var ot = L,
    sa = ot;
  function ia() {
    (this.__data__ = new sa()), (this.size = 0);
  }
  var ca = ia;
  function ua(t) {
    var e = this.__data__,
      a = e.delete(t);
    return (this.size = e.size), a;
  }
  var fa = ua;
  function la(t) {
    return this.__data__.get(t);
  }
  var pa = la;
  function ga(t) {
    return this.__data__.has(t);
  }
  var ha = ga,
    ya = typeof E == 'object' && E && E.Object === Object && E,
    ar = ya,
    va = ar,
    da = typeof self == 'object' && self && self.Object === Object && self,
    _a = va || da || Function('return this')(),
    m = _a,
    $a = m,
    ba = $a.Symbol,
    St = ba,
    nr = St,
    or = Object.prototype,
    Ta = or.hasOwnProperty,
    Aa = or.toString,
    J = nr ? nr.toStringTag : void 0;
  function Sa(t) {
    var e = Ta.call(t, J),
      a = t[J];
    try {
      t[J] = void 0;
      var o = !0;
    } catch {}
    var c = Aa.call(t);
    return o && (e ? (t[J] = a) : delete t[J]), c;
  }
  var wa = Sa,
    ma = Object.prototype,
    Ca = ma.toString;
  function Ia(t) {
    return Ca.call(t);
  }
  var Ma = Ia,
    sr = St,
    xa = wa,
    Pa = Ma,
    ja = '[object Null]',
    Ea = '[object Undefined]',
    ir = sr ? sr.toStringTag : void 0;
  function ka(t) {
    return t == null ? (t === void 0 ? Ea : ja) : ir && ir in Object(t) ? xa(t) : Pa(t);
  }
  var st = ka;
  function Oa(t) {
    var e = typeof t;
    return t != null && (e == 'object' || e == 'function');
  }
  var Y = Oa,
    Da = st,
    Ga = Y,
    Fa = '[object AsyncFunction]',
    Ba = '[object Function]',
    La = '[object GeneratorFunction]',
    Ua = '[object Proxy]';
  function Na(t) {
    if (!Ga(t)) return !1;
    var e = Da(t);
    return e == Ba || e == La || e == Fa || e == Ua;
  }
  var cr = Na,
    Ka = m,
    Ra = Ka['__core-js_shared__'],
    Ha = Ra,
    wt = Ha,
    ur = (function () {
      var t = /[^.]+$/.exec((wt && wt.keys && wt.keys.IE_PROTO) || '');
      return t ? 'Symbol(src)_1.' + t : '';
    })();
  function Va(t) {
    return !!ur && ur in t;
  }
  var za = Va,
    qa = Function.prototype,
    Wa = qa.toString;
  function Ja(t) {
    if (t != null) {
      try {
        return Wa.call(t);
      } catch {}
      try {
        return t + '';
      } catch {}
    }
    return '';
  }
  var fr = Ja,
    Ya = cr,
    Xa = za,
    Za = Y,
    Qa = fr,
    tn = /[\\^$.*+?()[\]{}|]/g,
    rn = /^\[object .+?Constructor\]$/,
    en = Function.prototype,
    an = Object.prototype,
    nn = en.toString,
    on = an.hasOwnProperty,
    sn = RegExp(
      '^' +
        nn
          .call(on)
          .replace(tn, '\\$&')
          .replace(/hasOwnProperty|(function).*?(?=\\\()| for .+?(?=\\\])/g, '$1.*?') +
        '$',
    );
  function cn(t) {
    if (!Za(t) || Xa(t)) return !1;
    var e = Ya(t) ? sn : rn;
    return e.test(Qa(t));
  }
  var un = cn;
  function fn(t, e) {
    return t == null ? void 0 : t[e];
  }
  var ln = fn,
    pn = un,
    gn = ln;
  function hn(t, e) {
    var a = gn(t, e);
    return pn(a) ? a : void 0;
  }
  var O = hn,
    yn = O,
    vn = m,
    dn = yn(vn, 'Map'),
    mt = dn,
    _n = O,
    $n = _n(Object, 'create'),
    it = $n,
    lr = it;
  function bn() {
    (this.__data__ = lr ? lr(null) : {}), (this.size = 0);
  }
  var Tn = bn;
  function An(t) {
    var e = this.has(t) && delete this.__data__[t];
    return (this.size -= e ? 1 : 0), e;
  }
  var Sn = An,
    wn = it,
    mn = '__lodash_hash_undefined__',
    Cn = Object.prototype,
    In = Cn.hasOwnProperty;
  function Mn(t) {
    var e = this.__data__;
    if (wn) {
      var a = e[t];
      return a === mn ? void 0 : a;
    }
    return In.call(e, t) ? e[t] : void 0;
  }
  var xn = Mn,
    Pn = it,
    jn = Object.prototype,
    En = jn.hasOwnProperty;
  function kn(t) {
    var e = this.__data__;
    return Pn ? e[t] !== void 0 : En.call(e, t);
  }
  var On = kn,
    Dn = it,
    Gn = '__lodash_hash_undefined__';
  function Fn(t, e) {
    var a = this.__data__;
    return (this.size += this.has(t) ? 0 : 1), (a[t] = Dn && e === void 0 ? Gn : e), this;
  }
  var Bn = Fn,
    Ln = Tn,
    Un = Sn,
    Nn = xn,
    Kn = On,
    Rn = Bn;
  function U(t) {
    var e = -1,
      a = t == null ? 0 : t.length;
    for (this.clear(); ++e < a; ) {
      var o = t[e];
      this.set(o[0], o[1]);
    }
  }
  (U.prototype.clear = Ln),
    (U.prototype.delete = Un),
    (U.prototype.get = Nn),
    (U.prototype.has = Kn),
    (U.prototype.set = Rn);
  var Hn = U,
    pr = Hn,
    Vn = ot,
    zn = mt;
  function qn() {
    (this.size = 0), (this.__data__ = { hash: new pr(), map: new (zn || Vn)(), string: new pr() });
  }
  var Wn = qn;
  function Jn(t) {
    var e = typeof t;
    return e == 'string' || e == 'number' || e == 'symbol' || e == 'boolean' ? t !== '__proto__' : t === null;
  }
  var Yn = Jn,
    Xn = Yn;
  function Zn(t, e) {
    var a = t.__data__;
    return Xn(e) ? a[typeof e == 'string' ? 'string' : 'hash'] : a.map;
  }
  var ct = Zn,
    Qn = ct;
  function to(t) {
    var e = Qn(this, t).delete(t);
    return (this.size -= e ? 1 : 0), e;
  }
  var ro = to,
    eo = ct;
  function ao(t) {
    return eo(this, t).get(t);
  }
  var no = ao,
    oo = ct;
  function so(t) {
    return oo(this, t).has(t);
  }
  var io = so,
    co = ct;
  function uo(t, e) {
    var a = co(this, t),
      o = a.size;
    return a.set(t, e), (this.size += a.size == o ? 0 : 1), this;
  }
  var fo = uo,
    lo = Wn,
    po = ro,
    go = no,
    ho = io,
    yo = fo;
  function N(t) {
    var e = -1,
      a = t == null ? 0 : t.length;
    for (this.clear(); ++e < a; ) {
      var o = t[e];
      this.set(o[0], o[1]);
    }
  }
  (N.prototype.clear = lo),
    (N.prototype.delete = po),
    (N.prototype.get = go),
    (N.prototype.has = ho),
    (N.prototype.set = yo);
  var vo = N,
    _o = ot,
    $o = mt,
    bo = vo,
    To = 200;
  function Ao(t, e) {
    var a = this.__data__;
    if (a instanceof _o) {
      var o = a.__data__;
      if (!$o || o.length < To - 1) return o.push([t, e]), (this.size = ++a.size), this;
      a = this.__data__ = new bo(o);
    }
    return a.set(t, e), (this.size = a.size), this;
  }
  var So = Ao,
    wo = ot,
    mo = ca,
    Co = fa,
    Io = pa,
    Mo = ha,
    xo = So;
  function K(t) {
    var e = (this.__data__ = new wo(t));
    this.size = e.size;
  }
  (K.prototype.clear = mo),
    (K.prototype.delete = Co),
    (K.prototype.get = Io),
    (K.prototype.has = Mo),
    (K.prototype.set = xo);
  var Po = K;
  function jo(t, e) {
    for (var a = -1, o = t == null ? 0 : t.length; ++a < o && e(t[a], a, t) !== !1; );
    return t;
  }
  var Eo = jo,
    ko = O,
    Oo = (function () {
      try {
        var t = ko(Object, 'defineProperty');
        return t({}, '', {}), t;
      } catch {}
    })(),
    Do = Oo,
    gr = Do;
  function Go(t, e, a) {
    e == '__proto__' && gr ? gr(t, e, { configurable: !0, enumerable: !0, value: a, writable: !0 }) : (t[e] = a);
  }
  var hr = Go,
    Fo = hr,
    Bo = er,
    Lo = Object.prototype,
    Uo = Lo.hasOwnProperty;
  function No(t, e, a) {
    var o = t[e];
    (!(Uo.call(t, e) && Bo(o, a)) || (a === void 0 && !(e in t))) && Fo(t, e, a);
  }
  var yr = No,
    Ko = yr,
    Ro = hr;
  function Ho(t, e, a, o) {
    var c = !a;
    a || (a = {});
    for (var l = -1, i = e.length; ++l < i; ) {
      var p = e[l],
        g = o ? o(a[p], t[p], p, a, t) : void 0;
      g === void 0 && (g = t[p]), c ? Ro(a, p, g) : Ko(a, p, g);
    }
    return a;
  }
  var ut = Ho;
  function Vo(t, e) {
    for (var a = -1, o = Array(t); ++a < t; ) o[a] = e(a);
    return o;
  }
  var zo = Vo;
  function qo(t) {
    return t != null && typeof t == 'object';
  }
  var X = qo,
    Wo = st,
    Jo = X,
    Yo = '[object Arguments]';
  function Xo(t) {
    return Jo(t) && Wo(t) == Yo;
  }
  var Zo = Xo,
    vr = Zo,
    Qo = X,
    dr = Object.prototype,
    ts = dr.hasOwnProperty,
    rs = dr.propertyIsEnumerable,
    es = vr(
      (function () {
        return arguments;
      })(),
    )
      ? vr
      : function (t) {
          return Qo(t) && ts.call(t, 'callee') && !rs.call(t, 'callee');
        },
    as = es,
    ns = Array.isArray,
    Ct = ns,
    ft = { exports: {} };
  function os() {
    return !1;
  }
  var ss = os;
  ft.exports,
    (function (t, e) {
      var a = m,
        o = ss,
        c = e && !e.nodeType && e,
        l = c && !0 && t && !t.nodeType && t,
        i = l && l.exports === c,
        p = i ? a.Buffer : void 0,
        g = p ? p.isBuffer : void 0,
        f = g || o;
      t.exports = f;
    })(ft, ft.exports);
  var _r = ft.exports,
    is = 9007199254740991,
    cs = /^(?:0|[1-9]\d*)$/;
  function us(t, e) {
    var a = typeof t;
    return (e = e ?? is), !!e && (a == 'number' || (a != 'symbol' && cs.test(t))) && t > -1 && t % 1 == 0 && t < e;
  }
  var fs = us,
    ls = 9007199254740991;
  function ps(t) {
    return typeof t == 'number' && t > -1 && t % 1 == 0 && t <= ls;
  }
  var $r = ps,
    gs = st,
    hs = $r,
    ys = X,
    vs = '[object Arguments]',
    ds = '[object Array]',
    _s = '[object Boolean]',
    $s = '[object Date]',
    bs = '[object Error]',
    Ts = '[object Function]',
    As = '[object Map]',
    Ss = '[object Number]',
    ws = '[object Object]',
    ms = '[object RegExp]',
    Cs = '[object Set]',
    Is = '[object String]',
    Ms = '[object WeakMap]',
    xs = '[object ArrayBuffer]',
    Ps = '[object DataView]',
    js = '[object Float32Array]',
    Es = '[object Float64Array]',
    ks = '[object Int8Array]',
    Os = '[object Int16Array]',
    Ds = '[object Int32Array]',
    Gs = '[object Uint8Array]',
    Fs = '[object Uint8ClampedArray]',
    Bs = '[object Uint16Array]',
    Ls = '[object Uint32Array]',
    v = {};
  (v[js] = v[Es] = v[ks] = v[Os] = v[Ds] = v[Gs] = v[Fs] = v[Bs] = v[Ls] = !0),
    (v[vs] =
      v[ds] =
      v[xs] =
      v[_s] =
      v[Ps] =
      v[$s] =
      v[bs] =
      v[Ts] =
      v[As] =
      v[Ss] =
      v[ws] =
      v[ms] =
      v[Cs] =
      v[Is] =
      v[Ms] =
        !1);
  function Us(t) {
    return ys(t) && hs(t.length) && !!v[gs(t)];
  }
  var Ns = Us;
  function Ks(t) {
    return function (e) {
      return t(e);
    };
  }
  var It = Ks,
    lt = { exports: {} };
  lt.exports,
    (function (t, e) {
      var a = ar,
        o = e && !e.nodeType && e,
        c = o && !0 && t && !t.nodeType && t,
        l = c && c.exports === o,
        i = l && a.process,
        p = (function () {
          try {
            var g = c && c.require && c.require('util').types;
            return g || (i && i.binding && i.binding('util'));
          } catch {}
        })();
      t.exports = p;
    })(lt, lt.exports);
  var Mt = lt.exports,
    Rs = Ns,
    Hs = It,
    br = Mt,
    Tr = br && br.isTypedArray,
    Vs = Tr ? Hs(Tr) : Rs,
    zs = Vs,
    qs = zo,
    Ws = as,
    Js = Ct,
    Ys = _r,
    Xs = fs,
    Zs = zs,
    Qs = Object.prototype,
    ti = Qs.hasOwnProperty;
  function ri(t, e) {
    var a = Js(t),
      o = !a && Ws(t),
      c = !a && !o && Ys(t),
      l = !a && !o && !c && Zs(t),
      i = a || o || c || l,
      p = i ? qs(t.length, String) : [],
      g = p.length;
    for (var f in t)
      (e || ti.call(t, f)) &&
        !(
          i &&
          (f == 'length' ||
            (c && (f == 'offset' || f == 'parent')) ||
            (l && (f == 'buffer' || f == 'byteLength' || f == 'byteOffset')) ||
            Xs(f, g))
        ) &&
        p.push(f);
    return p;
  }
  var Ar = ri,
    ei = Object.prototype;
  function ai(t) {
    var e = t && t.constructor,
      a = (typeof e == 'function' && e.prototype) || ei;
    return t === a;
  }
  var xt = ai;
  function ni(t, e) {
    return function (a) {
      return t(e(a));
    };
  }
  var Sr = ni,
    oi = Sr,
    si = oi(Object.keys, Object),
    ii = si,
    ci = xt,
    ui = ii,
    fi = Object.prototype,
    li = fi.hasOwnProperty;
  function pi(t) {
    if (!ci(t)) return ui(t);
    var e = [];
    for (var a in Object(t)) li.call(t, a) && a != 'constructor' && e.push(a);
    return e;
  }
  var gi = pi,
    hi = cr,
    yi = $r;
  function vi(t) {
    return t != null && yi(t.length) && !hi(t);
  }
  var wr = vi,
    di = Ar,
    _i = gi,
    $i = wr;
  function bi(t) {
    return $i(t) ? di(t) : _i(t);
  }
  var Pt = bi,
    Ti = ut,
    Ai = Pt;
  function Si(t, e) {
    return t && Ti(e, Ai(e), t);
  }
  var wi = Si;
  function mi(t) {
    var e = [];
    if (t != null) for (var a in Object(t)) e.push(a);
    return e;
  }
  var Ci = mi,
    Ii = Y,
    Mi = xt,
    xi = Ci,
    Pi = Object.prototype,
    ji = Pi.hasOwnProperty;
  function Ei(t) {
    if (!Ii(t)) return xi(t);
    var e = Mi(t),
      a = [];
    for (var o in t) (o == 'constructor' && (e || !ji.call(t, o))) || a.push(o);
    return a;
  }
  var ki = Ei,
    Oi = Ar,
    Di = ki,
    Gi = wr;
  function Fi(t) {
    return Gi(t) ? Oi(t, !0) : Di(t);
  }
  var jt = Fi,
    Bi = ut,
    Li = jt;
  function Ui(t, e) {
    return t && Bi(e, Li(e), t);
  }
  var Ni = Ui,
    pt = { exports: {} };
  pt.exports,
    (function (t, e) {
      var a = m,
        o = e && !e.nodeType && e,
        c = o && !0 && t && !t.nodeType && t,
        l = c && c.exports === o,
        i = l ? a.Buffer : void 0,
        p = i ? i.allocUnsafe : void 0;
      function g(f, d) {
        if (d) return f.slice();
        var A = f.length,
          C = p ? p(A) : new f.constructor(A);
        return f.copy(C), C;
      }
      t.exports = g;
    })(pt, pt.exports);
  var Ki = pt.exports;
  function Ri(t, e) {
    var a = -1,
      o = t.length;
    for (e || (e = Array(o)); ++a < o; ) e[a] = t[a];
    return e;
  }
  var Hi = Ri;
  function Vi(t, e) {
    for (var a = -1, o = t == null ? 0 : t.length, c = 0, l = []; ++a < o; ) {
      var i = t[a];
      e(i, a, t) && (l[c++] = i);
    }
    return l;
  }
  var zi = Vi;
  function qi() {
    return [];
  }
  var mr = qi,
    Wi = zi,
    Ji = mr,
    Yi = Object.prototype,
    Xi = Yi.propertyIsEnumerable,
    Cr = Object.getOwnPropertySymbols,
    Zi = Cr
      ? function (t) {
          return t == null
            ? []
            : ((t = Object(t)),
              Wi(Cr(t), function (e) {
                return Xi.call(t, e);
              }));
        }
      : Ji,
    Et = Zi,
    Qi = ut,
    tc = Et;
  function rc(t, e) {
    return Qi(t, tc(t), e);
  }
  var ec = rc;
  function ac(t, e) {
    for (var a = -1, o = e.length, c = t.length; ++a < o; ) t[c + a] = e[a];
    return t;
  }
  var Ir = ac,
    nc = Sr,
    oc = nc(Object.getPrototypeOf, Object),
    Mr = oc,
    sc = Ir,
    ic = Mr,
    cc = Et,
    uc = mr,
    fc = Object.getOwnPropertySymbols,
    lc = fc
      ? function (t) {
          for (var e = []; t; ) sc(e, cc(t)), (t = ic(t));
          return e;
        }
      : uc,
    xr = lc,
    pc = ut,
    gc = xr;
  function hc(t, e) {
    return pc(t, gc(t), e);
  }
  var yc = hc,
    vc = Ir,
    dc = Ct;
  function _c(t, e, a) {
    var o = e(t);
    return dc(t) ? o : vc(o, a(t));
  }
  var Pr = _c,
    $c = Pr,
    bc = Et,
    Tc = Pt;
  function Ac(t) {
    return $c(t, Tc, bc);
  }
  var Sc = Ac,
    wc = Pr,
    mc = xr,
    Cc = jt;
  function Ic(t) {
    return wc(t, Cc, mc);
  }
  var Mc = Ic,
    xc = O,
    Pc = m,
    jc = xc(Pc, 'DataView'),
    Ec = jc,
    kc = O,
    Oc = m,
    Dc = kc(Oc, 'Promise'),
    Gc = Dc,
    Fc = O,
    Bc = m,
    Lc = Fc(Bc, 'Set'),
    Uc = Lc,
    Nc = O,
    Kc = m,
    Rc = Nc(Kc, 'WeakMap'),
    Hc = Rc,
    kt = Ec,
    Ot = mt,
    Dt = Gc,
    Gt = Uc,
    Ft = Hc,
    jr = st,
    R = fr,
    Er = '[object Map]',
    Vc = '[object Object]',
    kr = '[object Promise]',
    Or = '[object Set]',
    Dr = '[object WeakMap]',
    Gr = '[object DataView]',
    zc = R(kt),
    qc = R(Ot),
    Wc = R(Dt),
    Jc = R(Gt),
    Yc = R(Ft),
    D = jr;
  ((kt && D(new kt(new ArrayBuffer(1))) != Gr) ||
    (Ot && D(new Ot()) != Er) ||
    (Dt && D(Dt.resolve()) != kr) ||
    (Gt && D(new Gt()) != Or) ||
    (Ft && D(new Ft()) != Dr)) &&
    (D = function (t) {
      var e = jr(t),
        a = e == Vc ? t.constructor : void 0,
        o = a ? R(a) : '';
      if (o)
        switch (o) {
          case zc:
            return Gr;
          case qc:
            return Er;
          case Wc:
            return kr;
          case Jc:
            return Or;
          case Yc:
            return Dr;
        }
      return e;
    });
  var Bt = D,
    Xc = Object.prototype,
    Zc = Xc.hasOwnProperty;
  function Qc(t) {
    var e = t.length,
      a = new t.constructor(e);
    return e && typeof t[0] == 'string' && Zc.call(t, 'index') && ((a.index = t.index), (a.input = t.input)), a;
  }
  var tu = Qc,
    ru = m,
    eu = ru.Uint8Array,
    au = eu,
    Fr = au;
  function nu(t) {
    var e = new t.constructor(t.byteLength);
    return new Fr(e).set(new Fr(t)), e;
  }
  var Lt = nu,
    ou = Lt;
  function su(t, e) {
    var a = e ? ou(t.buffer) : t.buffer;
    return new t.constructor(a, t.byteOffset, t.byteLength);
  }
  var iu = su,
    cu = /\w*$/;
  function uu(t) {
    var e = new t.constructor(t.source, cu.exec(t));
    return (e.lastIndex = t.lastIndex), e;
  }
  var fu = uu,
    Br = St,
    Lr = Br ? Br.prototype : void 0,
    Ur = Lr ? Lr.valueOf : void 0;
  function lu(t) {
    return Ur ? Object(Ur.call(t)) : {};
  }
  var pu = lu,
    gu = Lt;
  function hu(t, e) {
    var a = e ? gu(t.buffer) : t.buffer;
    return new t.constructor(a, t.byteOffset, t.length);
  }
  var yu = hu,
    vu = Lt,
    du = iu,
    _u = fu,
    $u = pu,
    bu = yu,
    Tu = '[object Boolean]',
    Au = '[object Date]',
    Su = '[object Map]',
    wu = '[object Number]',
    mu = '[object RegExp]',
    Cu = '[object Set]',
    Iu = '[object String]',
    Mu = '[object Symbol]',
    xu = '[object ArrayBuffer]',
    Pu = '[object DataView]',
    ju = '[object Float32Array]',
    Eu = '[object Float64Array]',
    ku = '[object Int8Array]',
    Ou = '[object Int16Array]',
    Du = '[object Int32Array]',
    Gu = '[object Uint8Array]',
    Fu = '[object Uint8ClampedArray]',
    Bu = '[object Uint16Array]',
    Lu = '[object Uint32Array]';
  function Uu(t, e, a) {
    var o = t.constructor;
    switch (e) {
      case xu:
        return vu(t);
      case Tu:
      case Au:
        return new o(+t);
      case Pu:
        return du(t, a);
      case ju:
      case Eu:
      case ku:
      case Ou:
      case Du:
      case Gu:
      case Fu:
      case Bu:
      case Lu:
        return bu(t, a);
      case Su:
        return new o();
      case wu:
      case Iu:
        return new o(t);
      case mu:
        return _u(t);
      case Cu:
        return new o();
      case Mu:
        return $u(t);
    }
  }
  var Nu = Uu,
    Ku = Y,
    Nr = Object.create,
    Ru = (function () {
      function t() {}
      return function (e) {
        if (!Ku(e)) return {};
        if (Nr) return Nr(e);
        t.prototype = e;
        var a = new t();
        return (t.prototype = void 0), a;
      };
    })(),
    Hu = Ru,
    Vu = Hu,
    zu = Mr,
    qu = xt;
  function Wu(t) {
    return typeof t.constructor == 'function' && !qu(t) ? Vu(zu(t)) : {};
  }
  var Ju = Wu,
    Yu = Bt,
    Xu = X,
    Zu = '[object Map]';
  function Qu(t) {
    return Xu(t) && Yu(t) == Zu;
  }
  var tf = Qu,
    rf = tf,
    ef = It,
    Kr = Mt,
    Rr = Kr && Kr.isMap,
    af = Rr ? ef(Rr) : rf,
    nf = af,
    of = Bt,
    sf = X,
    cf = '[object Set]';
  function uf(t) {
    return sf(t) && of(t) == cf;
  }
  var ff = uf,
    lf = ff,
    pf = It,
    Hr = Mt,
    Vr = Hr && Hr.isSet,
    gf = Vr ? pf(Vr) : lf,
    hf = gf,
    yf = Po,
    vf = Eo,
    df = yr,
    _f = wi,
    $f = Ni,
    bf = Ki,
    Tf = Hi,
    Af = ec,
    Sf = yc,
    wf = Sc,
    mf = Mc,
    Cf = Bt,
    If = tu,
    Mf = Nu,
    xf = Ju,
    Pf = Ct,
    jf = _r,
    Ef = nf,
    kf = Y,
    Of = hf,
    Df = Pt,
    Gf = jt,
    Ff = 1,
    Bf = 2,
    Lf = 4,
    zr = '[object Arguments]',
    Uf = '[object Array]',
    Nf = '[object Boolean]',
    Kf = '[object Date]',
    Rf = '[object Error]',
    qr = '[object Function]',
    Hf = '[object GeneratorFunction]',
    Vf = '[object Map]',
    zf = '[object Number]',
    Wr = '[object Object]',
    qf = '[object RegExp]',
    Wf = '[object Set]',
    Jf = '[object String]',
    Yf = '[object Symbol]',
    Xf = '[object WeakMap]',
    Zf = '[object ArrayBuffer]',
    Qf = '[object DataView]',
    tl = '[object Float32Array]',
    rl = '[object Float64Array]',
    el = '[object Int8Array]',
    al = '[object Int16Array]',
    nl = '[object Int32Array]',
    ol = '[object Uint8Array]',
    sl = '[object Uint8ClampedArray]',
    il = '[object Uint16Array]',
    cl = '[object Uint32Array]',
    h = {};
  (h[zr] =
    h[Uf] =
    h[Zf] =
    h[Qf] =
    h[Nf] =
    h[Kf] =
    h[tl] =
    h[rl] =
    h[el] =
    h[al] =
    h[nl] =
    h[Vf] =
    h[zf] =
    h[Wr] =
    h[qf] =
    h[Wf] =
    h[Jf] =
    h[Yf] =
    h[ol] =
    h[sl] =
    h[il] =
    h[cl] =
      !0),
    (h[Rf] = h[qr] = h[Xf] = !1);
  function gt(t, e, a, o, c, l) {
    var i,
      p = e & Ff,
      g = e & Bf,
      f = e & Lf;
    if ((a && (i = c ? a(t, o, c, l) : a(t)), i !== void 0)) return i;
    if (!kf(t)) return t;
    var d = Pf(t);
    if (d) {
      if (((i = If(t)), !p)) return Tf(t, i);
    } else {
      var A = Cf(t),
        C = A == qr || A == Hf;
      if (jf(t)) return bf(t, p);
      if (A == Wr || A == zr || (C && !c)) {
        if (((i = g || C ? {} : xf(t)), !p)) return g ? Sf(t, $f(i, t)) : Af(t, _f(i, t));
      } else {
        if (!h[A]) return c ? t : {};
        i = Mf(t, A, p);
      }
    }
    l || (l = new yf());
    var Z = l.get(t);
    if (Z) return Z;
    l.set(t, i),
      Of(t)
        ? t.forEach(function (w) {
            i.add(gt(w, e, a, w, t, l));
          })
        : Ef(t) &&
          t.forEach(function (w, S) {
            i.set(S, gt(w, e, a, S, t, l));
          });
    var Q = f ? (g ? mf : wf) : g ? Gf : Df,
      tt = d ? void 0 : Q(t);
    return (
      vf(tt || t, function (w, S) {
        tt && ((S = w), (w = t[S])), df(i, S, gt(w, e, a, S, t, l));
      }),
      i
    );
  }
  var ul = gt,
    fl = ul,
    ll = 1,
    pl = 4;
  function gl(t) {
    return fl(t, ll | pl);
  }
  var hl = gl;
  const yl = rr(hl),
    Jr = (t, e, a = !0) => {
      let o = 0;
      function c(l, i) {
        i && i.s && Array.isArray(i.s) && i.s.length === 4
          ? (l[o] && (i.s = [...l[o]]), o++)
          : i &&
            i.c &&
            i.c.k &&
            (Array.isArray(i.c.k) && typeof i.c.k[0] != 'number' ? c(l, i.c.k) : (l[o] && (i.c.k = l[o]), o++));
        for (const p in i) typeof i[p] == 'object' && c(l, i[p]);
        return i;
      }
      return c(t, a ? yl(e) : e);
    },
    vl = (t = [], e, a = !0) => {
      const o = [];
      return (
        t.forEach((c) => {
          o.push(I(c));
        }),
        Jr(o, a ? At(e) : e)
      );
    },
    dl = (t, e, a = !0) => {
      const o = I(t);
      if (!o) throw new Error('Proper colors must be used for target');
      function c(l, i) {
        if (i && i.s && Array.isArray(i.s) && i.s.length === 4) i.s = [...l];
        else if (i && i.c && i.c.k) Array.isArray(i.c.k) && typeof i.c.k[0] != 'number' ? c(l, i.c.k) : (i.c.k = l);
        else for (const p in i) typeof i[p] == 'object' && c(l, i[p]);
        return i;
      }
      return c(o, a ? At(e) : e);
    },
    M = (t) => Math.round(t * 1e3) / 1e3,
    H = 0.003,
    Ut = (t, e, a, o = !0) => {
      const c = I(t),
        l = I(e);
      if (!c || !l) throw new Error('Proper colors must be used for both source and target');
      function i(p, g, f) {
        if (f && f.s && Array.isArray(f.s) && f.s.length === 4)
          p[0] === f.s[0] && p[1] === f.s[1] && p[2] === f.s[2] && (f.s = [...g]);
        else if (f && f.c && f.c.k)
          Array.isArray(f.c.k) && typeof f.c.k[0] != 'number'
            ? i(p, g, f.c.k)
            : Math.abs(p[0] - M(f.c.k[0])) < H &&
              Math.abs(p[1] - M(f.c.k[1])) < H &&
              Math.abs(p[2] - M(f.c.k[2])) < H &&
              (f.c.k = g);
        else if (f.g && f.g.k && f.g.k.k && f.g.k.k.length % 4 === 0)
          for (let d = 0; d < f.g.k.k.length; d += 4)
            Math.abs(M(p[0]) - M(f.g.k.k[d + 1])) < H &&
              Math.abs(M(p[1]) - M(f.g.k.k[d + 2])) < H &&
              Math.abs(M(p[2]) - M(f.g.k.k[d + 3])) < H &&
              ((f.g.k.k[d + 1] = g[0]), (f.g.k.k[d + 2] = g[1]), (f.g.k.k[d + 3] = g[2]), (f.g.k.k[d + 4] = g[3]));
        else for (const d in f) typeof f[d] == 'object' && i(p, g, f[d]);
        return f;
      }
      return i(c, l, o ? At(a) : a);
    },
    _l = (t, e) => t.reduce((a, [o, c]) => Ut(o, c, a), e),
    $l = (t, e, a) => {
      let o = { ...a };
      return (
        t.forEach((c, l) => {
          const i = e[l % e.length];
          o = Ut(c, i, o);
        }),
        o
      );
    };
  (_.colorify = vl),
    (_.convertColorToLottieColor = I),
    (_.convertLottieColorToHex = ke),
    (_.convertLottieColorToRgb = Qt),
    (_.convertLottieColorToRgba = Tt),
    (_.flatten = dl),
    (_.getColors = tr),
    (_.getUniqueColors = Oe),
    (_.modifyColors = Jr),
    (_.replaceColor = Ut),
    (_.replaceColors = _l),
    (_.replaceColorsLoop = $l),
    Object.defineProperty(_, Symbol.toStringTag, { value: 'Module' });
});
